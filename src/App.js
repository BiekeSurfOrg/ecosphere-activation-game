    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg p-8 max-w-md w-full text-center">
        <div className="mb-6">
          ...
          <p className="text-gray-600">Ти сканира всички QR кодове! Вземи своята награда!</p>
        </div>
        <div className="space-y-4">
          <p className="text-lg font-semibold text-gray-700 mb-4">Избери своята награда:</p>
          <button
    ...
    <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
      ...
      <div className="text-center">
        ...
        <p className="text-lg font-semibold text-gray-700 mb-4">
          Имаш още {spins} завъртания
        </p>
        {lastResult !== null && (
          <div className="mb-4 p-3 bg-green-100 border border-green-300 rounded-lg">
            <p className="text-green-700 font-semibold">
              🎉 Спечели {lastResult} кредита!
            </p>
          </div>
        )}
        <button
        ...
        >
          {isSpinning ? 'Завъртане...' : '🎲 Завърти колелото'}
        </button>
      </div>
    </div>
  );
...
const App = () => {
  ...
  const processScan = useCallback(async (qrData) => {
    if (scannedLocations.some(loc => loc.locationId === qrData.locationId)) {
      setScanResultView({
        success: false,
        message: 'Вече си сканирал този QR код.',
        additionalInfo: 'Намери следващия код, за да продължиш.'
      });
      return;
    }
      ...
      <div className="max-w-md mx-auto">
        ...
        {scannedLocations.length < 3 && (
          <div className="bg-white bg-opacity-20 rounded-lg shadow-lg p-6 mb-6 text-center">
            <p>
              Сканирай следващия QR код с камерата на телефона си, за да продължиш приключението.
            </p>
          </div>
        )}
        <WheelSpinner
